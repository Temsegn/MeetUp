...
import { roomsService, type WorkspaceRoom } from '../../../services/rooms/rooms.service';
...
function StatusBadge({ status }: { status: WorkspaceRoom['status'] }) {
  const styles: Record<WorkspaceRoom['status'], string> = {
...
export function RoomsSettingsPanel({ onCreateRoom }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const rows = await roomsService.list(activeWorkspace.workspaceId);
      setRooms(rows);
    } catch (err) {
  ...
  useEffect(() => {
    const state = location.state as { createdRoomMessage?: string } | null;
    if (state?.createdRoomMessage) {
      setMessage(state.createdRoomMessage);
        ...
        <section className="rounded-[14px] border border-[#E2E7ED] bg-white shadow-[0_1px_3px_rgba(15,23,42,0.04)]">
          <div className="flex flex-col gap-3 border-b border-[#E2E7ED] px-4 pt-4 pb-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:px-5">
            <div className="min-w-0 shrink-0">
              ...
              <p className="mt-1 text-[12px] leading-snug text-[#6F7C8C]">
                Manage and organize all meeting rooms.
              </p>
            </div>
            ...
            <table className="w-full table-fixed border-collapse text-left">
              ...
              <thead>
                <tr className="border-b border-[#E2E7ED]/70 text-[11px] font-semibold text-[#6F7B8C]">
                  <th className="px-3 py-4 font-semibold sm:px-4">Room Name</th>
                  <th className="px-2 py-4 font-semibold">Room ID</th>
                  <th className="px-2 py-4 font-semibold">Capacity</th>
                  <th className="px-2 py-4 font-semibold">Created By</th>
              ...
              <tbody>
                {pageRows.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-8 text-center text-[12px] text-[#8A94A6]">
                  ...
                  pageRows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-2.5 text-[12px] text-[#475569]">{row.capacity}</td>
                      <td className="px-2 py-2.5">
                        <div className="flex min-w-0 items-center gap-2">
                          <UserAvatar
                            name={row.createdByName}
                            avatarUrl={row.createdByAvatarUrl}
                            avatarColor={row.createdByAvatarColor}
                            size="sm"
                          />
                          <div className="min-w-0">
                            <p className="truncate text-[12px] font-semibold text-[#151D2B]">
                        ...
                        {menuKey === row.id ? (
                          ...
                          >
                            <button
                              ...
                              onClick={() => {
                                setMenuKey(null);
                                setMessage(`${row.name} · ${row.roomId}`);
                              }}
                            >
                              View
                            </button>
          ...
          <div className="grid grid-cols-1 items-center gap-3 border-t border-[#E2E7ED] px-4 py-3 sm:grid-cols-[1fr_auto_1fr] sm:px-5">
            <p className="text-[11px] text-[#6F7C8C] sm:justify-self-start">
              Showing {from} to {to} of {filtered.length} rooms
            </p>
